              ...
              <div class="intro-y box">
                ...
                >
                  <h2 class="font-medium text-base mr-auto">
                    Edit 19/20 week Prenantal/Antenatal Visit for {{ data.prenatal.name }}
                  </h2>
                ...
                <div class="p-5">
                  <form class="grid grid-cols-12 gap-6" @submit.prevent="submit">
                    ...
                    <div class="col-span-12 xl:col-span-4 mt-3">
                      <label for="type_of_ultrasound_scan" class="form-label"
                        >Type of ultrasound scan</label
                      >
                      <select
                        id="type_of_ultrasound_scan"
                        class="form-control"
                        v-model="form.type_of_ultrasound_scan"
                      >
                        <option value="">Select type of scan</option>
                        <option value="dating scan">dating scan</option>
                        <option value="nuchal translucency scan">
                          nuchal translucency scan
                        </option>
                        <option value="morphology scan">morphology scan</option>
                      </select>
                      <div
                        class="text-danger font-medium text-xs mt-1"
                        v-if="errors.type_of_ultrasound_scan"
                      >
                        {{ errors.type_of_ultrasound_scan[0] }}
                      </div>
                    </div>
                    <div class="col-span-12 xl:col-span-6 mt-3">
                      ...
                      <textarea
                        ...
                        rows="5"
                        class="form-control"
                        v-model="form.scan_result"
                      ></textarea>
                      <div
                        class="text-danger font-medium text-xs mt-1"
                        v-if="errors.scan_result"
                      >
                        {{ errors.scan_result[0] }}
                      </div>
                    </div>
                    <div class="col-span-12 xl:col-span-6 mt-3">
    ...
    methods: {
      ...
      getPrenatal19Or20Week() {
        axios
          ...
          .then((response) => (this.form = response.data.prenatal_19_20_week))
          .catch((error) => {
            console.log(error);
            if (error.response.status == 401) {
              this.$router.push({
      ...
      submit() {
        this.loading = true;
        axios
          .patch("/api/prenatal-19-or-20-week/" + this.key, this.form)
          .then(() => {
            this.$router.push({
            ...
            Toast.fire({
              icon: "success",
              title: "Prenatal 19/20 week added successfully",
            });
          })
