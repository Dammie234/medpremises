            ...
            <div class="intro-y box">
              ...
              >
                <h2 class="font-medium text-base mr-auto">
                  Edit 26/27 week Prenantal/Antenatal Visit for {{ data.prenatal.name }}
                </h2>
  ...
  methods: {
    ...
    getPrenatal26Or27Week() {
      axios
        ...
        .then((response) => (this.form = response.data.prenatal_26_27_week))
        .catch((error) => {
          console.log(error);
          if (error.response.status == 401) {
            this.$router.push({
    ...
    submit() {
      this.loading = true;
      axios
        .patch("/api/prenatal-26-27-week/" + this.key, this.form)
        .then(() => {
          this.$router.push({
          ...
          Toast.fire({
            icon: "success",
            title: "Prenatal 22 week updated successfully",
          });
        })
