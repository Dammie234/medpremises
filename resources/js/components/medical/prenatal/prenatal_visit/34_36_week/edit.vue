              ...
              <div class="intro-y box">
                ...
                <div class="p-5">
                  <form class="grid grid-cols-12 gap-6" @submit.prevent="submit">
                    ...
                    <div class="col-span-12 xl:col-span-4 mt-2">
                      <label for="tt_doses" class="form-label">Tetanus Toxoid (TT) Doses</label>
                      <select
                        id="tt_doses"
                        class="form-control"
                        v-model="form.tt_doses"
                      >
                        <option value="">Select a tetanus toxoid doses</option>
                      ...
                      <div
                        class="text-danger font-medium text-xs mt-1"
                        v-if="errors.tt_doses"
                      >
                        {{ errors.tt_doses[0] }}
                      </div>
                    </div>
                    <div class="col-span-12 xl:col-span-4 mt-2">
                      <label for="tt_date" class="form-label">Tetanus Toxoid (TT) Date</label>
                      <input
                        id="tt_date"
                        type="date"
                        class="form-control"
                        v-model="form.tt_date"
                      />
                      <div
                        class="text-danger font-medium text-xs mt-1"
                        v-if="errors.tt_date"
    ...
    methods: {
      ...
      getPrenatal34Or36Week() {
        axios
          ...
          .then((response) => (this.form = response.data.prenatal_34_36_week))
          .catch((error) => {
            console.log(error);
            if (error.response.status == 401) {
              this.$router.push({
      ...
      submit() {
        this.loading = true;
        axios
          .patch("/api/prenatal-34-36-week/" + this.key, this.form)
          .then(() => {
            this.$router.push({
            ...
            Toast.fire({
              icon: "success",
              title: "Prenatal 34/36 week updated successfully",
            });
          })
