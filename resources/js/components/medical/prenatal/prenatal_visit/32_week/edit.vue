            ...
            <div class="intro-y box">
              ...
              >
                <h2 class="font-medium text-base mr-auto">
                  Edit 32 week Prenantal/Antenatal Visit for {{ data.prenatal.name }}
                </h2>
              ...
              <div class="p-5">
                <form class="grid grid-cols-12 gap-6" @submit.prevent="submit">
                  ...
                  <div class="col-span-12 xl:col-span-6 mt-2">
                    <label for="baby_heartbeat" class="form-label">Baby Heartbeat</label>
                    <textarea
                      id="baby_heartbeat"
                      rows="5"
                      class="form-control"
                  ...
                  <div class="col-span-12 xl:col-span-4 mt-2">
                    <label for="fansidar_doses" class="form-label">Fansidar Doses</label>
                    <select
                      id="fansidar_doses"
                      class="form-control"
                      v-model="form.fansidar_doses"
                    >
                      <option value="">Select a fansidar doses</option>
                    ...
                    <div
                      class="text-danger font-medium text-xs mt-1"
                      v-if="errors.fansidar_doses"
                    >
                      {{ errors.fansidar_doses[0] }}
                    </div>
                  </div>
                  <div class="col-span-12 xl:col-span-4 mt-2">
                    <label for="fansidar_date" class="form-label">Fansidar Date</label>
                    <input
                      id="fansidar_date"
                      type="date"
                      class="form-control"
                      v-model="form.fansidar_date"
                    />
                    <div
                      class="text-danger font-medium text-xs mt-1"
                      v-if="errors.fansidar_date"
                    >
                      {{ errors.fansidar_date[0] }}
                    </div>
                  </div>
  ...
  methods: {
    ...
    getPrenatal32Week() {
      axios
        ...
        .then((response) => (this.form = response.data.prenatal_32_week))
        .catch((error) => {
          console.log(error);
          if (error.response.status == 401) {
            this.$router.push({
    ...
    submit() {
      this.loading = true;
      axios
        .patch("/api/prenatal-32-week/" + this.key, this.form)
        .then(() => {
          this.$router.push({
          ...
          Toast.fire({
            icon: "success",
            title: "Prenatal 32 week updated successfully",
          });
        })
