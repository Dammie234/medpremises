            ...
            <div class="intro-y box">
              ...
              >
                <h2 class="font-medium text-base mr-auto">
                  Edit 22 week Prenantal/Antenatal Visit for {{ data.prenatal.name }}
                </h2>
  ...
  methods: {
    ...
    getPrenatal22Week() {
      axios
        ...
        .then((response) => (this.form = response.data.prenatal_22_week))
        .catch((error) => {
          console.log(error);
          if (error.response.status == 401) {
            this.$router.push({
    ...
    submit() {
      this.loading = true;
      axios
        .patch("/api/prenatal-22-week/" + this.key, this.form)
        .then(() => {
          this.$router.push({
