<template>
    <div class="main">
        ...
        <div class="flex">
            ...
            <div class="content">
                <!-- BEGIN: Top Bar -->
                <top-bar title="Drug"></top-bar>
                <!-- END: Top Bar -->

                <div class="grid grid-cols-12 gap-6" v-if="add_drug == true">
                    <div class="intro-y col-span-12 xxl:col-span-12">
                        <div class="intro-y box">
                            <div class="flex items-center p-5 border-b border-gray-200 dark:border-dark-5">
                                <h2 class="font-medium text-base mr-auto">Add Drug</h2>
                            </div>
                            <form class="input p-5" @submit.prevent="store">
                                <div class="grid grid-cols-12 gap-x-5" :disabled="loading">
                                    ...
                                    <div class="col-span-12 xl:col-span-6 mt-3">
                                        ...
                                        <input id="name" type="text" class="form-control" placeholder="Input drug name"
                                            v-model="form.name" />
                                    ...
                                    <div class="col-span-12 xl:col-span-3 mt-3">
                                        <label for="prescription_slip" class="form-label">Does Drug Require Prescription
                                            Slip</label>
                                        ...
                                        <div class="text-danger font-medium text-xs mt-1" v-if="errors.prescription_slip">
                                    ...
                                    <div class="col-span-12 xl:col-span-4 mt-3">
                                        ...
                                        <input id="manufacturer" type="text" class="form-control"
                                            placeholder="Input drug manufacturer" v-model="form.manufacturer" />
                                ...
                                <button class="btn btn-secondary" @click="addDrug">Close</button>
                            </form>
                        </div>
                    </div>
                </div>
                            ...
                            <div class="grid grid-cols-12 gap-x-5 p-5 border-b border-slate-200/60">
                                ...
                                <div class="col-span-12 xl:col-span-8">
                                    <div class="flex flex-col sm:flex-row gap-x-5">
                                        <div class="w-full">
                                            <select v-model="category" class="form-control" id="" v-if="drugs.length > 1">
                                        ...
                                        <div class="w-full">
                                            <input v-if="drugs.length > 1" class="form-control" type="text" v-model="shelf"
                                                @keyup="onChangeShelf" placeholder="Seach Shelf Name" />
                                        </div>
                                        <div class="w-full">
                                            <input v-if="drugs.length > 1" class="form-control" type="text" v-model="search"
                                                placeholder="Seach Drug Name" />
                                        </div>
                                        <button class="btn btn-primary w-60 text-xs" @click="addDrug">
                                            Add Drug
                                        </button>
                                    </div>
                                </div>
                                        ...
                                        <div v-if="drugs.length != 0">
                                            <div class="overflow-x-auto">
                                                <table class="table table-xs table-bordered">
                                                    ...
                                                    <tbody>
                                                        <tr class="text-xs" v-for="(drug, index) in filterSearch"
                                                            :key="index">
                                                            <td>{{ index + 1 }}</td>
                                                            <td>{{ drug.name }}</td>
                                                            ...
                                                            <td>
                                                                <button v-if="drug.prescription_slip == false"
                                                                    class="bg-danger rounded text-white px-3 py-2 mr-1 mb-2 text-xs">
                                                                    No
                                                                </button>
                                                                <button v-else
                                                                    class="bg-success rounded text-white px-3 py-2 text-xs">
                                                                    Yes
                                                                </button>
                                                            </td>
                                                            <td>{{ drug.delivery_name }}</td>
                                                            <td>{{ drug.added_by }}</td>
                                                            <td>{{ drug.updated_at | moment("from", "now") }}</td>
                                                            <td class="whitespace-nowrap">
                                                                <router-link :to="{
                                                                    name: 'edit-drug',
                                                                    params: { id: drug.uniqueId },
                                                                }" class="btn btn-primary">
                                                                    <i class="fa fa-edit"></i>
                                                                </router-link>
                                                                <button @click="deleteDrug(drug.uniqueId)"
        ...
        filterSearch() {
            if (this.category) {
                return this.drugs.filter((drug) => {
                    return drug.category.match(this.category);
                });
            } else if (this.shelf) {
                let str = this.shelf;
                let string = str.charAt(0).toUpperCase() + str.slice(1);
                return this.drugs.filter((drug) => {
                    return drug.shelf_name.match(string);
                });
            } else {
                let str = this.search;
                let string = str.charAt(0).toUpperCase() + str.slice(1);
                    ...
                    axios
                        ...
                        .then(() => {
                            this.drugs = this.drugs.filter((drug) => {
                                return drug.uniqueId != id;
                            });
                            this.$router.push({
                                name: "drug",
                            });
                            this.getDrugs();
                            Swal.fire("Deleted!", "A drug has been deleted.", "success");
                        })
                        .catch((error) => {
                            this.error = true;
                            console.log(error);
                            ...
                            Swal.fire(
                                "Not Deleted!",
                                "You can not delete this drug because the total quantity is not zero..",
                                "error"
                            );
                        });
                }
