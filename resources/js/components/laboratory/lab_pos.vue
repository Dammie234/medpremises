                  ...
                  <div class="tabs-content" v-if="active === 0">
                    <form class="input" @submit.prevent="processOrder">
                      <div class="p-5" :disabled="loading">
                        ...
                        <div class="mt-3">
                          <label
                            >Were Medical Laboratory Tests/Diagnostic prescribed during
                            Medical Investigation?</label
                          >
                          <div class="flex flex-col sm:flex-row mt-2">
                            <div class="form-check mr-2">
                              <input
                                ...
                                v-model="form.medical_investigation"
                                value="Yes"
                              />
                        ...
                        <div class="mt-3" v-if="form.medical_investigation == 'Yes'">
                          <vue-single-select
                            name="maybe"
                            placeholder="pick the patient"
                            you-want-to-select-a-post="ok"
              ...
              <div class="p-5">
                <div class="grid grid-cols-12 gap-x-5">
                  ...
                  <div class="col-span-12 xl:col-span-8 mt-3">
                    <input
                      class="form-control mr-0 mb-3"
                      type="text"
                      v-model="email"
                      placeholder="Seach Email of Patient"
                    />
                    <div class="overflow-x-auto" v-if="this.email">
                      <table class="table table-sm table-bordered mb-3">
                        <thead>
                          <tr>
                            <th class="whitespace-nowrap">Name</th>
                            <th class="whitespace-nowrap">Email</th>
                            <th class="whitespace-nowrap">Investigation</th>
                            <th class="whitespace-nowrap">Facility Registered Name</th>
                          </tr>
                        </thead>
                        <tbody>
                          <tr
                            v-for="invest in filterSearchInvestigation"
                            :key="invest.id"
                          >
                            <td>{{ invest.name }}</td>
                            <td>{{ invest.email }}</td>
                            <td>{{ invest.investigation }}</td>
                            <td>{{ invest.registered_facility_name }}</td>
                          </tr>
                        </tbody>
                      </table>
                    </div>
...
export default {
  ...
  created() {
    ...
    this.getHospitalMedicalInvestigation();
    this.getCarts();
    Reload.$on("AfterAdd", () => {
      this.getCarts();
    });
  ...
  methods: {
    ...
    getHospitalMedicalInvestigation() {
      axios
        .get("/api/v1/hospital-medical-investigation", {
          headers: {
            Authorization: "Bearer " + this.token,
            Accept: "application/json",
          },
        })
        .then((response) => (this.investigations = response.data))
        .catch((error) => {
          console.log(error);
          if (error.response.status == 401) {
            this.$router.push({
