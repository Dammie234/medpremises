              ...
              <div class="p-5">
                <div v-if="data.invoices.length != 0">
                  <div class="overflow-x-auto">
                    ...
                    <table
                      class="table table-xs table-bordered my-5"
                      v-if="data.admission == true"
                    >
                      <caption class="mt-3">
                        Patient Deposit
                      </caption>
...
export default {
  ...
  computed: {
    computeTotal() {
      ...
      if (this.data.admission == true) {
        if (total >= this.data.deposit.paid_amount) {
          this.total = total - this.data.deposit.paid_amount;
          this.form.amount = this.total;
          return total;
        } else {
          this.balance = this.data.deposit.paid_amount - total;
          this.form.amount = 0;
          return total;
        }
      } else {
  ...
  methods: {
    ...
    processOrder() {
      ...
      this.form.key = this.$route.params.key;
      axios
        .patch("/api/process-hospital-order/" + this.token, this.form)
        .then(() => {
          this.$router.push({
            path: `/medical/patient-invoice/${this.key}`,
          });
          Toast.fire({
            icon: "success",
