    ...
    <div class="flex">
      ...
      <div class="content">
        ...
        <div class="intro-y box overflow-hidden mt-5">
          ...
          >
            <div class="px-5 py-10 sm:px-20 sm:py-20 flex md:flex-row flex-col">
              <div>
                <div class="text-primary font-semibold text-3xl">INVOICE</div>
                <div class="mt-2">
                  Receipt
                  <span class="font-medium">#{{ hospital_sale.invoice_number }}</span>
                </div>
                <div class="mt-1">
                  {{ hospital_sale.created_at | moment("MMM Do, YYYY") }}
                </div>
              </div>
              <div class="lg:text-right lg:ml-auto" v-if="hospital_sale.deposit == 1">
                <div class="text-lg font-medium text-primary mt-2">
                  Patient Deposit Payment
                </div>
              </div>
            </div>
            <div class="flex flex-col lg:flex-row px-5 sm:px-20 pt-2 pb-5 sm:pb-10">
              <div>
                <div class="text-base text-slate-500">Customer Details</div>
                <div class="text-lg font-medium text-primary mt-2">
                  {{ hospital_sale.name }}
                </div>
                <div class="mt-1">{{ hospital_sale.email }}</div>
                <div class="mt-1">{{ hospital_sale.phone }}</div>
              </div>
          ...
          <div class="px-5 sm:px-20 pb-10 sm:pb-20 flex flex-col-reverse sm:flex-row">
            ...
            <div
              ...
              v-if="hospital_sale.deposit == 1"
            >
              <div class="text-base text-slate-500">Amount Paid</div>
              <div class="text-xl text-primary font-medium mt-2">
                &#8358;{{ hospital_sale.amount | numeral("0,0.00") }}
              </div>
              <div class="text-base text-slate-500 mt-2">Balance to Pay</div>
              <div class="text-xl text-primary font-medium mt-2">
                &#8358;{{ (computeTotal - hospital_sale.amount) | numeral("0,0.00") }}
              </div>
            </div>
            <div class="text-center sm:text-right sm:ml-auto" v-else>
...
export default {
  ...
  methods: {
    ...
    getHospitalSale() {
      axios
        .get("/api/v1/hospital-sale/" + this.key, {
          headers: {
            Authorization: "Bearer " + this.token,
            Accept: "application/json",
          },
        })
        .then((response) => (this.hospital_sale = response.data))
        .catch((error) => {
          console.log(error);
          if (error.response.status == 401) {
            this.$router.push({
    ...
    confirmPayment() {
      axios
        .get("/api/v1/confirm-hospital-payment/" + this.key, {
          headers: {
            Authorization: "Bearer " + this.token,
            Accept: "application/json",
          },
